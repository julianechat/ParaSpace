...
mod.data <- mod.data %>% 
  ...
  mutate(tot_host = tot_fish - tot_Cyprinidae) %>% 
  mutate(inf_host = inf_fish - inf_Cyprinidae)#Create infected fish abundance column
...
mod.data <- mod.data %>% 
  ...
  mutate(prev_host = inf_host/tot_host) #Create prevalence column

## Variable selection ----

mod.data <- mod.data %>% 
  select(Sampling_ID, Lake, Watershed,
         inf_fish, tot_fish, prev_fish, inf_LeGi, tot_LeGi, inf_host, tot_host, prev_host, prev_LeGi, tot_Cyprinidae,
